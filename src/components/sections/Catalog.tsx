...
const carClasses = ['Все', 'Люкс', 'Бизнес', 'Спорт', 'Внедорожник'];
...
const Catalog = () => {
  ...
  useEffect(() => {
    contentApi
      .list<Car>('cars')
      .then((items) => setCars(items.filter((c) => c.is_active !== false)))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  const filtered = filter === 'Все' ? cars : cars.filter((c) => c.car_class === filter);
                ...
                <CardFooter className="flex items-center justify-between">
                  <div>
                    <span className="font-display text-2xl font-bold text-gold">
                      {Number(car.price_per_day).toLocaleString('ru-RU')} ₽
                    </span>
